...
const tabHeaders = JSON.stringify([
...
export const PieChartStyles = () => {
    ...
    const renderSeries = (series: any) => (<oj-chart-series color={series.id === 'Series 1' ? color1 : undefined} border-color={series.id === 'Series 1' ? borderColor1 : undefined} border-width={series.id === 'Series 1' ? borderWidth1 : undefined} pattern={series.id === 'Series 1' ? pattern1 : undefined} pie-slice-explode={series.id === 'Series 1' ? explode : undefined}/>);
    return (<div id="chart-container" class="oj-flex oj-sm-padding-1x oj-sm-flex-items-1">
      <div class="oj-flex-item">
        <oj-chart id="pieChart" type="pie" data={dataProvider} animation-on-data-change="auto" pie-center={pieCenter} style-defaults={styleDefaults} {...{ 'legend.position': 'bottom' }}>
      ...
      <demo-tabs class="oj-flex-item" headers={tabHeaders} onvalueChanged={handleSelectedTabValueChanged} value={selectedTab}>
        <div class="oj-sm-padding-1x">
          ...
          <oj-form-layout aria-controls="pieChart" max-columns={2}>
