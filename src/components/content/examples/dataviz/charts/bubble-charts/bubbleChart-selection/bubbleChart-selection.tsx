...
const data = JSON.parse(dataText as string) as BubbleDatum[];
export const BubbleChartSelection = () => {
  ...
  const idToItemMap = useMemo<Record<number, BubbleDatum | undefined>>(() => ({}), []);
  const selectionInfo = () => {
      let items = '';
...
return (
      <div id="chart-container">
            <oj-form-layout>
                    <demo-radioset-enum aria-controls="bubbleChart" onvalueChanged={handleSelectionValueValueChanged} value={selectionValue} labelHint="Selection" direction="row" enumValues={["none","single","multiple"]} />
                </oj-form-layout>
            <oj-chart id="bubbleChart" type="bubble" selectionMode={selectionValue} selection={selectedItemsValue} data={dataProvider} animationOnDisplay="auto" animationOnDataChange="auto" onselectionChanged={selectionListener}>
...
export default BubbleChartSelection;
