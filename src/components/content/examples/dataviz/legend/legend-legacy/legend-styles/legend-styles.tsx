...
type LegendItemTextStyle = NonNullable<ComponentProps<'oj-legend'>['textStyle']>;
type LegendSectionTitleStyle = NonNullable<ComponentProps<'oj-legend'>['sectionTitleStyle']>;
...
export const LegendStyles = (): JSX.Element => {
  ...
  const [pattern, setPattern] = useState<LegendPattern>('none');
  ...
  const itemTemplateRenderer = ($current: { data: FruitItem; index: number }): JSX.Element => (
    <oj-legend-item
      ...
      pattern={$current.index === 4 ? pattern : undefined}
      symbolType={$current.index === 4 ? symbolType : undefined}
    />
  );

  const sectionTemplateRenderer = (): JSX.Element => <oj-legend-section text={sectionTitle} />;
        ...
        <div>
          ...
          <oj-form-layout aria-controls="legend1" maxColumns={2}>
            ...
            <demo-select-enum
              value={pattern}
              labelHint="pattern"
              enumValues={["none", "smallChecker", "largeTriangle", "largeDiagonalRight", "largeDiamond", "smallCrosshatch", "largeDiagonalLeft", "largeCrosshatch", "smallDiagonalLeft", "smallDiagonalRight", "smallDiamond", "smallTriangle", "largeChecker"]}
              onvalueChanged={(event: JetElementCustomEvent<LegendPattern>) => setPattern(event.detail.value)}
            />
            <demo-select-enum
              value={symbolType}
