...
type ItemActionEvent = ojListView.ojItemAction<Task['taskId'], Task>;
type ItemTemplateContext = ojListView.ItemTemplateContext<Task['taskId'], Task>;
type SelectedChangedEvent = ojListView.selectedChanged<Task['taskId'], Task>;
...
const createSelectedKeys = (keys: Task['taskId'][] = []) =>
  new KeySetImpl<Task['taskId']>(keys) as SelectedKeySet;
...
>[0];
type ReorderEvent = ojListView.ojReorder;
...
const getSelectedIds = (tasks: Task[], selected: SelectedKeySet) => {
  const selectedKeys = (selected as ImmutableKeySet<Task['taskId']>).keys;
  if (selectedKeys.all) {
    return tasks
...
export const ListViewOverviewListView = () => {
  ...
  const dataProvider = useMemo(
    () =>
      new ArrayDataProvider<Task['taskId'], Task>(tasks, {
  ...
  const handleSelectedChanged = (event: SelectedChangedEvent) => {
    const nextSelected = (event.detail.value as SelectedKeySet | null) ?? createSelectedKeys();
    setSelected(nextSelected);
  };

  const handleSelectorSelectedKeysChanged = (event: SelectorSelectedKeysChangedEvent) => {
    setSelected((event.detail.value as SelectedKeySet | null) ?? createSelectedKeys());
  };

  const handleItemAction = (event: ItemActionEvent) => {
    const key = event.detail.context.data.taskId;
  ...
  const handleDraftFieldChanged =
    (field: DraftTaskField) => (event: ValueChangedEvent) => {
  ...
  const handleReorder = (event: ReorderEvent) => {
    ...
    const destinationKey = referenceContext?.key;
    ...
    const sourceIndex = nextTasks.findIndex((task) => task.taskId === sourceKey);
    const destinationIndex = nextTasks.findIndex((task) => task.taskId === destinationKey);
    ...
    const adjustedDestinationIndex = nextTasks.findIndex((task) => task.taskId === destinationKey);
    const insertIndex =
      event.detail.position === 'after' ? adjustedDestinationIndex + 1 : adjustedDestinationIndex;
    nextTasks.splice(insertIndex, 0, sourceTask);
    ...
    <oj-list-item-layout>
      ...
      {!isEditItem ? (
        <span
          slot="trailing"
          role="presentation"
          class="oj-sm-margin-2x-start oj-sm-align-items-flex-end oj-listview-drill-icon"
        />
      ) : null}
      {isEditItem ? (
        <span slot="action" role="presentation" class="oj-listview-drag-handle" />
      ) : null}
  ...
  const renderEditItem = (item: ItemTemplateContext) => <li>{renderTaskLayout(item, true)}</li>;
        ...
        >
          <oj-list-view
            ...
            {...{
              ...
              'gridlines.bottom': 'visible',
              'item.enter-key-focus-behavior': 'none',
  ...
  const renderTaskPanel = () => (
    <section class="demo-task-panel" aria-live="polite">
      <div class="demo-task-panel__toolbar">
        <oj-button id="backButton" chroming="outlined" label="Back" onojAction={handleClosePanel}>
