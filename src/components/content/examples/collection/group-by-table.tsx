...
const dataprovider: MutableArrayTreeDataProvider<Dept["id"], Dept> =
  new MutableArrayTreeDataProvider(dataArray, "id", { keyAttributeScope: "global" });
...
// TODO Find out proper Type for item
const setRowSelectable = (item:any) => {
...
return (
  ...
  {(row.item.metadata.treeDepth === 0) && (
    <td colSpan={6} aria-label={row.item.data.accText} class="oj-sm-padding-0-start">
  ...
  {(row.item.metadata.treeDepth === 1) && (
    ...
    <td>
      <span>
        {row.item.data.probability}
      </span>
    </td>
    <td>
      <span>
        {row.item.data.description}
      </span>
    </td>
    <td>
      <span>
        {row.item.data.account}
      </span>
    </td>
    <td>
      <span>
