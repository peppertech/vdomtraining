...
const createToolbarItems = (
  onToolbarItemAction: (value: string) => void,
  isLoading: boolean,
...
): ToolbarItems => [
  {
    type: "button",
    key: "button1",
    onAction: () => onToolbarItemAction("button1 Action"),
    label: "Undo",
    display: "icons",
  ...
  },
  {
    type: "button",
    key: "button2",
    onAction: () => onToolbarItemAction("button2 Action"),
    label: "Redo",
    display: "icons",
    ...
    items: [
      {
        ...
        label: "Zoom In",
        key: "zoomin",
        onAction: () => onToolbarItemAction("Menu Button Item Action"),
  ...
  },
  {
    ...
    onAction: () => onToolbarItemAction("Split Menu Button Action"),
    label: "Edit",
    items: [
...
export const ToolbarItemActionscorepack = () => {
  const [toolbarItemAction, setToolbarItemAction] = useState("(No action performed)");
  ...
  const toolbarItems = useMemo(
    () => createToolbarItems(setToolbarItemAction, loading, progressHandler),
    [loading]
  );
  ...
  return (
    <div id="toolbar-container">
      ...
      <div class="oj-typography-body-md oj-typography-bold oj-sm-margin-5x-top">
        ToolbarAction, Last action item pressed: <span id="toolbarItemAction">{toolbarItemAction}</span>
      </div>
    </div>
  );
};

export default ToolbarItemActionscorepack;
