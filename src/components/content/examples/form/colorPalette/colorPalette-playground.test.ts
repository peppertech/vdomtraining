(() => {
  ...
  const demoSource = readFileSync(join(directory, "colorPalette.tsx"), "utf8");
  const registrationSource = readFileSync(join(directory, "index.tsx"), "utf8");

  match(demoSource, /export default function ColorPalette\(\)/);
  match(registrationSource, /playground:\s*\{/);
  match(registrationSource, /initialSource:\s*colorPalettePlaygroundSource/);
