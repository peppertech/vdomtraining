...
import {
  ...
  labelEdgeOptions,
  type InputDateTextLabelEdge,
  type RadiosetValueChangedEvent,
} from "./inputDateText-shared";

export default function InputDateTextWidthExample() {
  const [labelEdge, setLabelEdge] =
    useState<InputDateTextLabelEdge>("inside");

  const handleLabelEdgeChanged = useCallback(
    (event: RadiosetValueChangedEvent) => {
      const nextValue = event.detail.value;
      if (isInputDateTextLabelEdge(nextValue)) {
        setLabelEdge(nextValue);
      }
    },
    [],
  );
  ...
  return (
    <div id="inputDateTextWidth">
      ...
      <oj-c-form-layout labelEdge={labelEdge}>
        <oj-c-input-date-text
          id="id7"
          labelHint="width and max-width attributes are not defined"
          help={{
