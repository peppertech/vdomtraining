...
export const ValidationUsecasesValidateMethod = () => {
  ...
  const [userNameMessagesCustom, setUserNameMessagesCustom] =
    useState<InputTextMessagesCustom>([]);
  ...
  const handleSubmit = async () => {
    const usernameResult = await userNameRef.current?.validate();
    const passwordResult = await passwordRef.current?.validate();
  ...
  const handleAddCustomMessage = () => {
    const usernameMsg: InputTextMessage = {
      summary: '',
      detail: 'Warning messages are not considered invalid.',
      severity: 'warning'
    };
    setUserNameMessagesCustom([usernameMsg]);
  };
  ...
  return (
    <oj-form-layout id="validation-usecase">
      <oj-c-input-text
        ...
        messagesCustom={userNameMessagesCustom}
...
export default ValidationUsecasesValidateMethod;
