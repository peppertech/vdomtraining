...
export const tabbarTabroutingRecipe = (
  <>
    <p>
      The tabbar has 4 tabs, one for each route. Selecting a tab updates the route parameter and the
      content of the panel with the value of the route.
    </p>
    <p>
      Using the browser back button, the route parameter will transition back to the previous state,
      in effect, changing the selected tab and the panel content.
    </p>
    <p>
    ...
    <ul>
      ...
      <li>
        Define the contents of the panel with the current route path and selected route detail to
        show the human-readable label of the route.
      </li>
      <li>
    ...
    <ul>
      ...
      <li>
        ...
        &lt;oj-tab-bar&gt; template will iterate through this data provider to create the individual
        tabs.
      </li>
      <li>Read the route value from the URL hash and store it in component state.</li>
      <li>Listen for hashchange events so browser back and forward update the selected tab.</li>
      <li>Update the hash parameter when the tab bar selection changes.</li>
    </ul>
  </>
);
